...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride = [16,], anchor_scales = [8, 16, 32]):
    ...
    #anchor_scales = [8, 16, 32] around a center, relative distance
...
#__C.TRAIN.RPN_MIN_SIZE = 16
    ...
    height, width = rpn_cls_prob_reshape.shape[1:3]#feat map

    # the first set of _num_anchors channels are bg probs
    # the second set are the fg probs, which we want
    # (1, H, W, A)
#rpn_cls_prob_reshape [1 h w Ax2]->[1 h w A 2], get [1 h w A 1] as BackGround->[1 h w Ax1]
    ...
    bbox_deltas = rpn_bbox_pred#1 H W Ax4
    ...
    # Enumerate all shifts,feat map to original image
    shift_x = np.arange(0, width) * _feat_stride#1xwidth
    shift_y = np.arange(0, height) * _feat_stride#1xheight
    shift_x, shift_y = np.meshgrid(shift_x, shift_y)#heightxwidth heightxwidth
#[wxh,4]
#[[0 0 0 0]
#[16 0 16 0]
#[32 0 32 0]
...
#relative anchors[1,9,4]+points[hxw,1,4]=[hxw,9,4]
    ...
    scores = scores.reshape((-1, 1))#[1*h*w*A,1]
